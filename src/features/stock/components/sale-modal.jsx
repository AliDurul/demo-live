...
import { selectBrands, selectBrandsStatus, selectProducts, selectProductsStatus } from "../stockSlice"
...
export function SaleModal({ open, onOpenChange, selectedSale }) {
    ...
    function onSubmit(data) {
        ...
        if (isEditMode) {
            updateStock('sales', selectedSale._id, data)
        } else {
            createStock('sales', data)
        }
        onOpenChange(false)
    }
    ...
    useEffect(() => {
        ...
        // getStock('brands')
        // getStock('products')
        getStockResources(['brands', 'products'])
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            {/* <DialogTrigger render={<Button>Create New Sale</Button>} /> */}
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>{isEditMode ? "Edit Sale" : "Create New Sale"}</DialogTitle>
                    <DialogDescription>
                        {
                            isEditMode ? 'You can update sale for the company you work with.' : " You can create a new sale for the company you work with."
                        }
                    </DialogDescription>
                </DialogHeader>
                <div className="-mx-4 no-scrollbar max-h-[50vh] overflow-y-auto px-4">
                    <form id="sale-create-frm" onSubmit={form.handleSubmit(onSubmit)}>
                        <FieldGroup>
                            <Controller
                                name="brandId"
                ...
                <DialogFooter>
                    ...
                    <Button type="submit" form="sale-create-frm">{isSubmitting ? "Saving..." : isEditMode ? "Update Sale" : "Create Sale"}</Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
    )
